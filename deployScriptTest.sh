...
__unused=$(awslocal ec2 authorize-security-group-ingress --group-id "$db_sg_id" --protocol tcp --port 3306 --source-group "$web_server_sg_id")
...
__unused=$(awslocal rds create-db-subnet-group --db-subnet-group-name "$db_subnet_group_name" --db-subnet-group-description "subnet group for main database" --subnet-ids '["$private_subnet_1_id", "$private_subnet_2_id"]' --query "DBSubnetGroup"."SubnetGroupStatus" --output text)
