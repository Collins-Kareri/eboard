...
notify 'warning' '\nThis script use awslocal cli, please make sure it is configured\b'

notify "info" "Preparing script"
default_region=$(awslocal ec2 describe-availability-zones --query 'AvailabilityZones[0].[RegionName]' --output text)

# get availability
primary_az=$(awslocal ec2 describe-availability-zones --region $default_region --query "AvailabilityZones"[0]."ZoneName" --output text)

secondary_az=$(awslocal ec2 describe-availability-zones --region $default_region --query "AvailabilityZones"[1]."ZoneName" --output text)
...
createSubnet() {
	echo $(awslocal ec2 create-subnet --vpc-id $vpc_id --cidr-block $1 --availability-zone $2 --tag-specifications "ResourceType=subnet,Tags=[{Key=Name,Value=$3}]" --query Subnet.SubnetId --output text)
}
...
__unused=$(awslocal ec2 authorize-security-group-ingress --group-id $web_server_sg_id --protocol tcp --port 80 --cidr 0.0.0.0/0)
...
db_sg_id=$(awslocal ec2 create-security-group --group-name $db_security_group_name --description "database SG" --vpc-id $vpc_id --tag-specification "ResourceType=security-group,Tags=[{Key=Name,Value=$db_security_group_name}]" --query "GroupId" --output text)

notify 'info' 'Adding database access rule to db security group'
__unused=$(awslocal ec2 authorize-security-group-ingress --group-id "$db_sg_id" --protocol tcp --port 3306 --source-group "$web_server_sg_id" 2>/dev/null)
...
__unused=$(awslocal rds create-db-subnet-group --db-subnet-group-name "$db_subnet_group_name" --db-subnet-group-description "subnet group for main database" --subnet-ids "$private_subnet_1_id" "$private_subnet_2_id" --query "DBSubnetGroup"."DBSubnetGroupName" --output text)
...
db_host=$(awslocal rds describe-db-instances --db-instance-identifier ${db_arn} --query 'DBInstances[0].Endpoint.Address' --output text)

# setup the webserver

web_server_Ip=$(awslocal ec2 describe-instances --instance-ids $web_server_id --query "Reservations"[0]."Instances"[0]."PublicIpAddress" --output text)
